# Solution for auto-generating forms

Inspired from the core principles of [Netlify Cms](https://www.netlifycms.org/), this system was built and adapted to better fit this project specificities.

The following documentation explains how form pages can be auto-generated using a simple `.yml` file, in concordance with the form layouts as well as the translation documents found in the `_data/i18n/` folder. However, there are still some limits to the automation process up till now, for instance, the javascript has to be done manually.
